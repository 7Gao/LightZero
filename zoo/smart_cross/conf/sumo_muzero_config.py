{'env': {'sumocfg_path': 'sumo_wj3/rl_wj.sumocfg', 
         'gui': False, 
         'inference': False, 
         ...
         'reward': {'use_centralized_reward': True, 
                    'reward_type': {'queue_len': 1.0}}}}
...
smartcross_muzero_config = dict(
    ...
    env=dict(
        config_path='/home/qi/Workspace/github/DI-smartcross/smartcross/envs/sumo_wj3_default_config.yaml',
        ...
        muzero=True,
        tls=['ftddj_wjj', 'ftddj_frj', 'htddj_gsndj'],
        obs=dict(obs_type=['phase', 'lane_pos_vec'], 
                 lane_grid_num=10, 
                 queue_len_ratio=7.5, 
                 use_centralized_obs=True, 
                 padding=False),
        action=dict(action_type='change', 
                    use_multi_discrete=True),
        reward=dict(use_centralized_reward=True,
                    reward_type=dict(queue_len=1.0)),

    ),
    policy=dict(
        model=dict(
            observation_shape=442,
